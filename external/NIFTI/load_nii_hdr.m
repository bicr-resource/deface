...
function [hdr, filetype, fileprefix, machine] = load_nii_hdr(fileprefix)
   ...
   fid = fopen_ascii_encoding(fn, 'r', machine);
...
function [ fid ] = fopen_ascii_encoding(file, mode, machine)

    if vb_matlab_version('>=', '7.2')
        fid = fopen(file, mode, machine, 'US-ASCII');
    else
        fid = fopen(file, mode, machine);
    end
